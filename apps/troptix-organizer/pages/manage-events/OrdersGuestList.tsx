...
export default function OrdersGuestList({ orders }) {
  ...
  function filterList(value: string) {
    if (value === "" || value === undefined) {
      ...
      setGuests(originalGuestList.filter(guest =>
            ...
            return (
              <View key={index}>
                <View marginT-16 row>
                  <View flex>
                    <View>
                      <Text>Order ID: {String(guest.orderId).toUpperCase()}</Text>
                      <Text>Ticket ID: {String(guest.id).toUpperCase()}</Text>
                      <Text style={{ textDecorationLine: guest.status === TicketStatus.NOT_AVAILABLE ? 'line-through' : 'none' }}>
                  ...
                  <View style={{ justifyContent: 'center', alignItems: 'center' }} marginL-8>
                    <Button onPress={() => checkIn(guest, index)} label={buttonText} labelStyle={{ fontSize: 14 }} outline borderRadius={10} />
                  </View>
