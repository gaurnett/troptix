...
import {
  ComplementaryOrder,
  ComplementaryTicket,
  generateComplementaryOrder,
} from '@/hooks/types/Order';
import {
  PostOrdersRequest,
  PostOrdersType,
  useCreateComplementaryOrder,
...
export default function TicketCompForm({ event, ticketTypes, onClose }) {
  ...
  const [complementaryOrder, setComplementaryOrder] =
    useState<ComplementaryOrder>(generateComplementaryOrder(event));
  const [ticketTypeOptions, setTicketTypeOptions] = useState<Map<string, any>>(
    new Map()
  );
  ...
  async function sendComplementaryTicket() {
    if (!selectedTicket) {
      if (canShowMessage) {
        ...
        message
          .error('Please select ticket type')
  ...
  return (
    <div className="md:max-w-md">
      {contextHolder}
      <Form className="" name="comp-form" onFinish={sendComplementaryTicket}>
        <h3
        ...
        <div className="md:flex md:justify-between">
          <div className="mb-4 md:mr-4 w-full">
            <CustomInput
              value={complementaryOrder.firstName}
              name={'firstName'}
              id={'firstName'}
              label={'First Name'}
              type={'text'}
          ...
          <div className="mb-4 md:ml-4 w-full">
            <CustomInput
              value={complementaryOrder.lastName}
              name={'lastName'}
              id={'lastName'}
              label={'Last Name'}
              type={'text'}
        ...
        <div className="flex flex-wrap -mx-3 mb-4">
          <div className="w-screen px-3">
            <CustomInput
              value={complementaryOrder.email}
              name={'email'}
              id={'email'}
              label={'Email'}
