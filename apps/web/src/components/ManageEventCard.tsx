...
export default function ManageEventCard({ event, showDivider = false }) {
  ...
  function renderMobileCard() {
    return (
      <div>
        ...
        {showDivider && <Divider dashed className="m-0 mt-2" />}
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="py-2 px-4 bg-white bg-opacity-90 absolute inset-x-0 bottom-0">
          ...
          <p className="text-green-700 text-sm">{priceString}</p>
          <p
            style={{
            ...
            className="text-gray-700 text-sm text-ellipsis"
          >
            {event.venue}
          </p>
          <p
            className={`${event.isDraft ? 'text-amber-900' : 'text-green-600'} text-amber-900 text-sm`}
          >
            Status: {event.isDraft ? 'Draft' : 'Published'}
          </p>
        </div>
