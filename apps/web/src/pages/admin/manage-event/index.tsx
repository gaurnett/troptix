...
export default function ManageEventPage() {
  ...
  useEffect(() => {
    async function updateEventPublishedState() {
      if (event !== undefined && publishButtonClicked) {
        const successMessage = event.isDraft ? "Successfully unpublished event." : "Successfully published event.";
  ...
  const items: TabsProps['items'] = [
    ...
    },
    {
      key: 'basic-info',
      label: 'Basic Info',
      children: <BasicInfoPage event={event} setEvent={setEvent} updateEvent={updateEvent} />,
    },
    {
      key: 'details',
      label: 'Details',
      children: <DetailsPage event={event} setEvent={setEvent} updateEvent={updateEvent} />,
    },
    {
      key: 'tickets',
      label: 'Tickets',
            ...
            <div className='flow-root'>
              <div className="flex my-2">
                ...
                <div className="my-auto">
                  <Button onClick={publishEvent} className="px-4 py-4 shadow-md items-center font-medium inline-flex">{event.isDraft ? "Publish Event" : "Set to draft"}</Button>
                </div>
              </div>
